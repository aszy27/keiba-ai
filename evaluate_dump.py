# evaluate_dump.py
# evaluate_main.py と同じバックテストを行い、レース単位の結果を CSV に保存して
# 「バックテストと実戦の差」を診断する。evaluate_main.py 自体は変更しない。
import pandas as pd
import numpy as np
import torch
import os
...
def main():
    ...
    out_horses = os.path.join(BASE_DIR, "result", "backtest_horses.csv")
    df_test_final[['race_id', 'race_date', 'horse_id', 'horse_number', 'rank', 'lgb_rank_score', 'lgb_prob_score',
                   'transformer_prob', 'final_score']].to_csv(out_horses, index=False, encoding='utf-8-sig')
    print(f"   -> 💾 馬単位のスコアを保存: {out_horses}")
    ...
    for rid, grp in tqdm(df_test_final.groupby('race_id'), desc="Result Check"):
        ...
        race_results.append({
            'race_id': str(rid),
            'ai_top3_umaban': '-'.join(str(int(x)) for x in sorted(grp.iloc[:3]['horse_number'].values)),
    ...
    out_csv = os.path.join(BASE_DIR, "result", "backtest_races.csv")
    df_res.to_csv(out_csv, index=False, encoding='utf-8-sig')
    print(f"\n💾 レース単位のバックテスト結果を保存: {out_csv}")
    diagnose(df_res)
    return
...
# =========================================================================
# 🩺 診断: バックテストの回収率は「少数の高配当」に依存していないか？
#          実戦(predict_past)と同じレースで買い目・自信度が一致しているか？
# =========================================================================
def diagnose(df_res, threshold=60.0, live_n=93, n_boot=20000, seed=0):
    rng = np.random.default_rng(seed)
    tgt = df_res[(df_res['confidence'] >= threshold) & df_res['has_return']].copy()
    tgt['pay'] = tgt['s3_payout'] * tgt['is_3puku_hit']
    n, hits = len(tgt), int(tgt['is_3puku_hit'].sum())
    print("\n" + "=" * 90)
    print(f"🩺 [診断1] 自信度{threshold:.0f}%以上: {n}R / 的中{hits} / 回収率 {tgt['pay'].sum() / n:.1f}%")
    hp = tgt.loc[tgt['is_3puku_hit'] == 1, 'pay'].sort_values(ascending=False)
    if len(hp):
        print(f"   的中時の払戻: 平均 {hp.mean():.0f}円 / 中央値 {hp.median():.0f}円 / 最大 {hp.max():.0f}円")
        print(f"   高配当トップ10: {hp.head(10).astype(int).tolist()}")
        for k in [0, 1, 3, 5, 10]:
            print(f"   上位{k:>2}本の高配当を除いた回収率: {hp.iloc[k:].sum() / n:6.1f}%")
    # 実戦と同じ本数(live_n)をランダムに買ったときの回収率の分布
    pays = tgt['pay'].values
    sims = np.array([rng.choice(pays, live_n, replace=True).sum() / live_n for _ in range(n_boot)])
    print(f"\n🩺 [診断2] {live_n}R だけ買ったときの回収率のばらつき（バックテスト分布から復元抽出 {n_boot}回）")
    for q in [5, 25, 50, 75, 95]:
        print(f"   {q:>2}%点: {np.percentile(sims, q):6.1f}%")
    print(f"   回収率100%未満になる確率: {(sims < 100).mean() * 100:.1f}%")
    print(f"   回収率 72.8%以下になる確率: {(sims <= 72.8).mean() * 100:.1f}%")

    # 実戦予想(predict_past/*.txt)との突き合わせ
    pdir = os.path.join(BASE_DIR, "predict_past")
    ...
    for f in os.listdir(pdir):
        if not f.endswith('.txt'):
            continue
        txt = open(os.path.join(pdir, f), encoding='utf-8', errors='ignore').read()
        for blk in txt.split('監督AI診断')[1:]:
            m_id = re.search(r'Race: (\d{12})', blk)
            m_c = re.search(r'自信度[^:：]*[:：]\s*([\d.]+)%', blk)
            if not (m_id and m_c):
                continue
            nums = re.findall(r'^\s*\d+\s+(\d+)\s+\S+\s+([\d.]+)', blk.split('-' * 20)[-1], re.M)
            top = sorted(int(u) for u, _ in nums[:3])
            rows.append({'race_id': m_id.group(1), 'live_conf': float(m_c.group(1)),
                         'live_top3': '-'.join(map(str, top))})
    if not rows:
        return
    live = pd.DataFrame(rows).drop_duplicates('race_id', keep='last')
    cmp_ = live.merge(df_res[['race_id', 'race_date', 'confidence', 'ai_top3_umaban']], on='race_id', how='inner')
    if cmp_.empty:
        print("\n🩺 [診断3] 実戦予想と重なるレースがバックテストにありません（data/test を更新してください）")
        return
    cmp_['same_top3'] = cmp_['live_top3'] == cmp_['ai_top3_umaban']
    cmp_['conf_diff'] = cmp_['live_conf'] - cmp_['confidence']
    both = ((cmp_['live_conf'] >= threshold) == (cmp_['confidence'] >= threshold)).mean() * 100
    print(f"\n🩺 [診断3] 実戦予想 vs バックテスト（同じ {len(cmp_)} レース）")
    print(f"   上位3頭が一致: {cmp_['same_top3'].mean() * 100:.1f}%")
    print(f"   自信度の差（実戦−バックテスト）: 平均 {cmp_['conf_diff'].mean():+.1f}pt / 絶対値平均 {cmp_['conf_diff'].abs().mean():.1f}pt")
    print(f"   閾値{threshold:.0f}%での購入判定が一致: {both:.1f}%")
    print("   ※ 一致率が低ければ predict_main と evaluate_main で特徴量がズレている（train/serving skew）")
    out_cmp = os.path.join(BASE_DIR, "result", "live_vs_backtest.csv")
    cmp_.sort_values('race_id').to_csv(out_cmp, index=False, encoding='utf-8-sig')
    print(f"   💾 突き合わせ結果: {out_cmp}")
